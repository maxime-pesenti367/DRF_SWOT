...
`copernicusmarine>=2.4.1` requires `numpy>=2.1.0` (confirmed via `pip install --dry-run`: *"copernicusmarine 2.4.1 depends on numpy>=2.1.0"*) — a hard conflict with the `numpy==1.26.4` pin above. You cannot have a working `copernicusmarine` and a working `geomstats`/DRF in the same venv. (Versions of `copernicusmarine` old enough to coexist with `numpy==1.26.4`, e.g. 2.3.0, predate that package's support for downloading sparse/SQLite-backed datasets as NetCDF at all — moot now anyway, since `copernicus_pipeline.py` no longer uses `.subset(file_format="netcdf")`; see below.)

`copernicus_pipeline.py` fetches via `copernicusmarine.read_dataframe()`, not `.subset()` — the latter's sparse-altimetry NetCDF-writer path has a genuine bug in `copernicusmarine` 2.4.1 (its own internal per-platform pivot occasionally produces a non-monotonic time index, and a subsequent `.sel(time=..., method="nearest")` call on it raises `ValueError: index must be monotonic increasing or decreasing` for certain satellite/date combinations — reproduced independent of pandas version, 2.3.3 vs 3.0.3, and confirmed the raw data itself is clean via `read_dataframe()` on the same query, so this is `copernicusmarine`-internal, not a project or environment issue). `_sparse_dataframe_to_dataset()` in `copernicus_pipeline.py` pivots the returned long-format table into an `xr.Dataset` itself, sidestepping the bug entirely and applying to every satellite fetch, not just the ones that happen to hit it.
...
python build_experiment_data.py --config configs/data/all_sats_1_day.yaml --plot     # step 1: fetch + split + tensorize a dataset -- separate fetch venv, see below; --plot saves a tracks_overview.png alongside the .pt files
...
- **`copernicus_pipeline.py`** — shared low-level pipeline used by both exp4 and exp5. `SATELLITE_DATASET_MAP` is nested `product_type` (`"nrt"` near-real-time / `"my"` multi-year reprocessed) → satellite name → resolution (`"1hz"`/`"5hz"`) → Copernicus product ID; satellite names are kept consistent across `nrt`/`my` where they refer to the same physical satellite/orbit (e.g. `"cryosat2"` is the current-orbit product in both), so switching `product_type` in a data config without changing the satellite list behaves predictably. `_resolve_satellites(config, product_type)` expands a data config's `satellites: all` into every satellite registered under that `product_type`, all at the config's single top-level `freq` key (an explicit `satellites: [{name, freq}, ...]` list is passed through unchanged, each entry keeping its own per-satellite freq); an unknown satellite name or a freq not available for that satellite+product_type is skipped with a warning rather than raising (needed since, e.g., none of the `my` products have a `5hz` variant). `fetch_and_store_satellites`/`load_experiment_dataset` download/cache per-satellite Zarr stores from Copernicus Marine, keyed by `product_type`+satellite+date-range — `zarr files/<product_type>/<sat_name>/<sat_name>_<freq>_<start>_<end>.zarr`, product_type folded into the path specifically so an `nrt` and `my` fetch of the same satellite+date-range never collide in the cache — independent of any "experiment name". Fetching goes through `copernicusmarine.read_dataframe()`, not `.subset(file_format="netcdf")` — see the venv section above for why; `_sparse_dataframe_to_dataset()` pivots the returned long-format table (`variable`/`value` columns, one row per observation) into an `xr.Dataset` itself, and normalizes the `time` column to naive `datetime64[ns]` (`pd.to_datetime(..., utc=True).dt.tz_convert(None).astype("datetime64[ns]")`) to match what the old NetCDF path's CF-decoding produced — needed so `xr.concat` in `combine_for_drf` doesn't choke on mixed dtypes between old cached Zarr stores and newly-fetched ones. `combine_for_drf` merges multiple satellites' data into one dataframe/xarray dataset. `store_tensors`/`get_drive_base_path` handle Drive-relative tensor storage — see the Data section above for `get_drive_base_path`'s platform-aware behaviour. `process_and_split_dataframe` is **not** used by exp5 — it bakes together a 2-way split with spatial z-score normalization that breaks `spherical_to_cartesian()`'s geometry (a known bug, deliberately not fixed in exp4 where it still exists — exp5's own tensorization was written from scratch to avoid it). `copernicus_pipeline_old.py` is a superseded variant (`open_dataset` instead of `.subset`) kept for reference.
- **`display_tracks.py`** — plotting/diagnostic utilities for visualizing satellite ground-track density (e.g. `get_spatial_density` computes an equal-area lat/lon histogram in km² for heatmaps). `display_1D_tracks` builds the 4-panel overview (time series, distribution, spatial scatter, density heatmap) used by both the notebooks and `build_experiment_data.py --plot`; pass `save_path` to save the figure with `fig.savefig` instead of the default `plt.show()`, so it also works from a plain terminal invocation with no interactive backend.
...
- **`build_experiment_data.py`** — step 1 of the exp5 pipeline. Takes a **data config** (`configs/data/*.yaml`: `product_type` (`nrt`/`my`, defaults to `nrt` if omitted — old configs need no changes), satellites (an explicit `[{name, freq}, ...]` list, or `satellites: all` + top-level `freq` to sweep every satellite under that product_type), date range, variables, list of named split strategies), orchestrates the existing fetch/load/combine pipeline from `copernicus_pipeline.py`, applies each configured split via `data_splits.py`, then normalizes/tensorizes itself (spatial: degrees→radians only, no z-scoring; temporal: Unix seconds z-scored using the *train* split's own mean/std; target: left raw). Saves one `.pt` file per split to `<drive_base>/pytorch tensors/<data-config-name>/<split-name>.pt` via `store_tensors`. Logs split sizes and warns (not errors) on empty splits. Optional `--plot` flag saves a `tracks_overview.png` (via `display_tracks.display_1D_tracks`) into that same `<drive_base>/pytorch tensors/<data-config-name>/` folder, alongside the `.pt` split files — a terminal-only substitute for eyeballing the fetched data in `drive_test.ipynb`, built from the already-fetched `data_dict` with no extra Copernicus calls.
...
- **`experiment_5.py`** — step 2 of the exp5 pipeline; the canonical training/eval script. Loads a **model config** (`configs/exp5/*.yaml`: which data config + split to use via `data.experiment_name`/`data.split_name`, model architecture, BO search settings, training settings, optionally `seed` and `training.max_parallel_models`), runs a Bayesian-optimization search against a real held-out val split, then retrains + saves the winning candidate(s). `training.early_stopping.patience` / `training.lr_scheduler.type` (`cosine` or `reduce_on_plateau`) / `training.gradient_clipping.max_norm` are opt-in (absent = old behaviour, unchanged) and, when any is set, route the run through `SphericalBayesianOptimizerSWOT` (see `spherical_uq_methods_SWOT.py` below) instead of the protected `DRF.spherical_uq_methods.SphericalBayesianOptimizer` (shared with exp3/exp4).

  After the search, the run identifies **two** candidates from `optimizer.search_history` — the round with the lowest `final_loss` (the actual BO objective) and, separately, the round with the lowest `val_rmse` — since the lowest-`final_loss` round isn't always the most accurate one by RMSE; if both criteria pick the same round this collapses to one candidate. Both (or the one) get retrained and saved via `_retrain_and_save_candidate` (a deliberate small duplication of `train_model_process`'s loop — needed because that shared function only ever returns predictions, never the model object itself), each scored against the real held-out test set (RMSE/NLPD/CRPS, plus `test_bias`/`test_variance_of_diff` — RMSE² = bias² + variance_of_diff, matching DUACS L4 altimetry-product validation practice). Each candidate's checkpoints, prediction tensors, `training_curve.csv/.png`, and whole-globe grid snapshot land in `results_dir/final_loss_winner/` and `results_dir/val_rmse_winner/` (or a single `results_dir/final_loss_and_val_rmse_winner/` when they coincide); `results.csv` has one row per candidate with a `selection_criterion` column. `search_history.csv`/`search_progress.png` cover the whole search (both winners marked, via index-based `is_winner`/`is_val_rmse_winner` columns) and live only in the top-level `results_dir` — not duplicated per-candidate, so there's exactly one copy to regenerate later. `search_history.csv` also gets `val_bias`/`val_variance_of_diff` when running through `SphericalBayesianOptimizerSWOT` (absent for protected-DRF-path runs). All three CSVs (`results.csv`, `search_history.csv`, `training_curve.csv`) are rounded to 6 significant figures before saving.

  Whole-globe grid snapshots (`final_mean.png`/`final_variance.png`, forward-passing the already-trained ensemble over a synthetic dense grid — no separate retrain needed, unlike exp4, since the weights are already saved) render at a fixed 2880×1440 (0.125°/pixel) with **no resampling** — pixel-exact via explicit `fig.add_axes()` positioning in pixel-fraction terms and `interpolation="none"` (`_NUM_LONGS`/`_NUM_LATS`/`_GRID_DPI`/`_BORDER_PX` constants; matplotlib's default figure sizing would otherwise silently resample the grid to fit a fixed pixel budget), with a small uniform white border. `replot_grid.py` keeps its own duplicate copy of this plotting code in sync manually (not shared between the two files). Results land in `results/<model-config-name>/`, name auto-derived from the config filename (never hand-typed).
- **`spherical_uq_methods_SWOT.py`** — SWOT-project-owned (**not** part of the protected `DeepRandomFeatures/src/DRF/` package), used only when a model config sets `training.early_stopping`/`lr_scheduler`/`gradient_clipping`. `train_model_process_swot()` duplicates `train_model_process`'s training loop, adding a per-epoch validation pass, gradient clipping (`torch.nn.utils.clip_grad_norm_`), LR scheduler stepping (`CosineAnnealingLR` or `ReduceLROnPlateau`), and best-epoch weight rollback (`copy.deepcopy(model.state_dict())`, restored once patience is exhausted — the same stopping rule is applied independently in both the BO search phase and the final retrain, not forced to the same epoch count across the two, since GPU training isn't run-to-run deterministic anyway). `SphericalBayesianOptimizerSWOT` subclasses the protected `SphericalBayesianOptimizer`, inheriting `__init__`/`optimize()` unchanged and overriding only `objective_function` to route through `train_model_process_swot` and record `val_bias`/`val_variance_of_diff` in `search_history`.
- **`plot_search_history.py`** — regenerates `search_progress.png`/`training_curve.png` from `search_history.csv`/`training_curve.csv` alone (`python plot_search_history.py --results-dir results/<config-name>`, pointed at the top-level results dir, not a candidate subfolder). `plot_search_progress` renders a 3×2 grid (final_loss / val_nlpd / val_rmse / val_bias / val_variance_of_diff / val_crps, val_rmse placed beside val_bias and above val_variance_of_diff) when `search_history.csv` has the SWOT-only bias/variance columns, else falls back to the original 4-panel layout; marks the final_loss-winning round (red star) and, only if genuinely different, the val_rmse-winning round (green star).
- **`replot_grid.py`** — standalone: regenerates a saved exp5 run's whole-globe grid PNGs from its checkpoints alone (`python replot_grid.py --results-dir results/<config-name>`), with zero retraining or BO search. Useful for updating plots for older runs, or after a plotting-code change, without repeating the expensive part. Re-seeds `torch.manual_seed(checkpoint["seed"])` immediately before reconstructing each model — `MaternRandomPhaseS2RFFLayer`'s random spherical basis (`RandomPhaseFeatureMap`, not an `nn.Module`) isn't part of `state_dict()`, so without this the reconstructed model combines the saved output-layer weights with an unrelated random basis, silently producing a plausible-looking but wrong (and non-deterministic run-to-run) grid. Batches the grid forward pass (`DataLoader`/`TensorDataset`, `_GRID_BATCH_SIZE = 8000`) to avoid a CUDA OOM at the 2880×1440 render resolution.
