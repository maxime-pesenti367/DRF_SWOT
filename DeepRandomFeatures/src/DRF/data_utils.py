...
def prepare_tensor_datasets(obs_data, test_locs):
    ...
    spatial_tensor_x = torch.tensor(spatial_X.copy(), dtype=torch.float32)
    #temporal_tensor_x = torch.Tensor(temporal_X)
    temporal_tensor_x = torch.tensor(temporal_X.copy(), dtype=torch.float32)
    #tensor_y = torch.Tensor(Y)
    tensor_y = torch.tensor(Y.copy(), dtype=torch.float32)
    ...
    #) / temporal_X_std
    normalized_spatial_test_X = (
    torch.tensor(spatial_test_X.copy(), dtype=torch.float32) - spatial_X_mean
    ) / spatial_X_std
    normalized_temporal_test_X = (
    torch.tensor(temporal_test_X.copy(), dtype=torch.float32) - temporal_X_mean
    ) / temporal_X_std
